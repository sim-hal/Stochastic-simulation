...
from src.util import RealFunction

if __name__ == "__main__":
    cdf = lambda x: 0 if x < 0 else 1 - 2/3 * np.exp(-x / 2) if x < 2 else 1
    inv_cdf: RealFunction = np.vectorize(lambda y: 0 if y < 1/3 else -2 * np.log((3 / 2) * (1 - y)) if y < 1 - 2/3 * np.exp(-1) else 2)
    Y = inverse_method(inv_cdf)
    samples = Y(1000)
